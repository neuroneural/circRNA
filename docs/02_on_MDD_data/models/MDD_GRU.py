...
"""
GRU models extracted from the MDD_preproc project, adapted to the ml4fmri
external-model API.

Source
------
MDD_preproc/mdd_classifier/models/gru.py       (GRUModel, GRUAttentionModel)
...
ICA component time courses (B, T, n_components). This is the model
`experiments_clf/unimodal_simple_models.sh` selects for the `time_series`
modality, with hidden_size=128, num_layers=2, bidirectional=True, dropout=0.1,
fc_hidden_dims=[64], lr=3e-4 — the defaults reproduced below. That run was
configured but never completed (no `time_series/` folder exists under
results/unimodal_simple_models).
...
Note on RNN pooling
-------------------
MDD_preproc takes `h_n[-1]` as the sequence summary. For a bidirectional GRU
...
exactly, or `pool="attention"` / `MDDGRUAttention` for attention pooling.
...
-----
    ...
    report = cvbench(DATA, LABELS,
                     models=["meanMLP"],
                     custom_models=[MDDGRU, MDDGRUAttention],
                     n_folds=5)

ml4fmri has no built-in GRU, so these class names collide with nothing.
"""
...
class MDDGRU(nn.Module):
    """
    TIME SERIES MODEL

    Configurable GRU with an optional MLP head, from MDD_preproc.
    ...
    def __init__(
        ...
        Args:
            ...
            hidden_size (int, hyperparameter): GRU hidden size. Defaults to 128.
            num_layers (int, hyperparameter): Number of GRU layers. Defaults to 2.
            bidirectional (bool, hyperparameter): Whether to use a bidirectional GRU. Defaults to True.
            dropout (float, hyperparameter): Dropout between GRU layers (ignored if num_layers == 1). Defaults to 0.1.
...
class MDDGRUAttention(MDDGRU):
    """
    TIME SERIES MODEL

    MDD_preproc's GRUAttentionModel: same GRU, but pooled with attention over
    ...
    def __init__(
        ...
        """Same arguments as MDDGRU, with `pool` fixed to "attention"."""
        super().__init__(
            input_size=input_size,
...
if __name__ == "__main__":
    B, T, D, C = 8, 120, 53, 2
    x = torch.randn(B, T, D)
    for cls in (MDDGRU, MDDGRUAttention):
