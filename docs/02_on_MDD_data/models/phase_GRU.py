...
"""
GRU models extracted from the phase_prediction project, adapted to the ml4fmri
external-model API.
...
1. Pin it with the `with_seq_len` helper (faithful to the original):
       ...
       from phase_GRU import PhaseBiGRU, with_seq_len
       Model = with_seq_len(PhaseBiGRU, DATA.shape[1])    # e.g. 140 TRs
...
* `output_size` is the number of classes: the head emits `output_size` logits
  ...
  (regression / BCE style) and `BiGRUAttention` had its output size hard-coded
  to 1.
...
class PhaseBiGRU(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            norm (str, hyperparameter): "batch" (original, BatchNorm1d over the time axis plus \
                BatchNorm1d on the FC output), "layer", or "none". "auto" picks "batch" when \
        ...
        if activation == "tanh":
            ...
            self.act = nn.ReLU()
        # the phase_prediction GRUs normalise the FC output a second time
        if norm == "batch":
...
class PhaseBiGRUAttention(PhaseBiGRU):
    """
    TIME SERIES MODEL

    phase_prediction's BiGRUAttention: same stack, but the sequence is reduced
...
if __name__ == "__main__":
    ...
    x = torch.randn(B, T, D)
    variants = {
        "PhaseBiGRU (seq_len pinned, flatten+batchnorm)": with_seq_len(PhaseBiGRU, T)(D, C),
        "PhaseBiGRU (length-agnostic, mean+layernorm)": PhaseBiGRU(D, C),
        "PhaseBiGRUAttention (seq_len pinned)": with_seq_len(PhaseBiGRUAttention, T)(D, C),
        "PhaseBiGRUAttention (length-agnostic)": PhaseBiGRUAttention(D, C),
    }
